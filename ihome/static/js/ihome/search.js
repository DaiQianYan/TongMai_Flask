...
$(document).ready(function(){
    ...
    $.get("/api/v1.0/areas", function(data){
        if ("0" == data.errno) {
            //用户从首页跳转到这个搜索页面时，可能选择了城区，所以尝试从url的查询字符串参数中提取用户选择的城区
            var areaId = queryData["aid"];
            //如果获取城区id的数据
            if (areaId) {
                //遍历从后端获取的城区信息，添加到页面中
                for (var i=0; i<data.data.length; i++) {
                    //从后端获取的areaId是整型，从url参数中获取到的字符串类型，所以将url参数中获取到的转为整型
            ...
            window.onscroll=function(){
                ...
                if(c-b<windowHeight+50){
                    //如果没有向后端发送查询房屋列表信息的请求
                    if (!house_data_querying) {
                        //将正在向后端查询房屋列表信息的标识设置为真
